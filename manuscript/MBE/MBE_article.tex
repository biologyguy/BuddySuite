...
\begin{table}[!t]
\tableparts{\caption{Runtime for each BuddySuite tool.\label{table:timeit}}}
      ...
      \\\botrule
      \end{tabular}}
{\tablenote{Over 140K RefSeq records were identified in GenBank using the following query: ``Nematoda''[Organism] AND biomol\_mrna[PROP] AND refseq[filter]. The results were downloaded in GenBank format and subsamples of 10, 100, 1000, and 10000 records were used to test the runtime performance of the BuddySuite tools. Runtime percentiles are expressed in seconds.}}
\end{table}
