...
]{\inits{S}\fnm{Sofia} \snm{de Pereira Barreira,}}
\author[
   addressref={aff1},
...
Whichever format is encountered last will be the format the records are written to the output in (in this case, FASTA), although this behaviour may be overridden with the `--output' modifier:
...
---------------------------------------------------------------------------------
DatabaseBuddy, on the other hand, has been implemented as a live-shell application. 
---------------------------------------------------------------------------------
...
\section*{Conclusions}
BuddySuite has been designed from the ground up to be an intuitive, extensible, and unified platform for routine command-line tasks performed on sequence, alignment, and phylogenetic tree files. While the individual tools are not generally novel in and of themselves, this is the first time that such a large suite of general-purpose bioinformatics utilities have been implemented purely in Python and packaged together under a flag-driven paradigm. Well designed and actively supported open-source tools are going to be invaluable over the coming years as an increasing number of biologists turn to the command line to analyze their data. We hope that BuddySuite will be widely adopted by the community and, thanks to the passive data-collection built into this project, we look forward to tailoring future development to the needs of our users.
...
\section*{Author's contributions}
  SRB is the lead developer of BuddySuite and wrote the manuscript, KEK contributed significantly to the code base and documentation, SPB generated the performance statistics for FIG.X, and ADB was involved in the design and coordination of the project. All authors read and approved the final manuscript.
